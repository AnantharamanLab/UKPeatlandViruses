...
virus.clusters.balmoral <- virus.clusters.balmoral %>%
  ...
  mutate(Title = factor(treatment, levels = c("NAT", "REST", "DAM")))
virus.clusters.balmoral$Title <- paste("Group: ", virus.clusters.balmoral$Cluster, "\n", virus.clusters.balmoral$Cluster.size, " viruses", sep = "")
...
ggplot(data = virus.clusters.balmoral, aes(x = factor(treatment, levels = c("NAT", "REST", "DAM")), y = MeanZS)) +
...
virus.clusters.bowness <- virus.clusters.bowness %>%
  ...
  mutate(Cluster.size = n_distinct(Virus)) %>%
  mutate(Title = factor(treatment, levels = c("NAT", "REST", "DAM")))
virus.clusters.bowness$Title <- paste("Group: ", virus.clusters.bowness$Cluster, "\n", virus.clusters.bowness$Cluster.size, " viruses", sep = "")
...
ggplot(data = virus.clusters.bowness, aes(x = factor(treatment, levels = c("NAT", "REST", "DAM")), y = MeanZS)) +
  geom_boxplot(aes(color = treatment), outlier.shape = NA) +
  scale_color_manual(
        values = c("NAT" = "#4DAF4A", "REST" = "#377EB8", "DAM" = "#E41A1C"),
        name = "Ecosystem\nhealth status",
...
virus.clusters.crocach <- virus.clusters.crocach %>%
  ...
  mutate(Cluster.size = n_distinct(Virus)) %>%
  mutate(Title = factor(treatment, levels = c("NAT", "REST", "DAM")))
virus.clusters.crocach$Title <- paste("Group: ", virus.clusters.crocach$Cluster, "\n", virus.clusters.crocach$Cluster.size, " viruses", sep = "")
...
```
    ...
    ##   Virus                   Cluster order treatment  MeanZS Title     Cluster.size
    ##   <chr>                     <int> <int> <fct>       <dbl> <chr>            <int>
    ## 1 BAr1A1B1C_vRhyme_bin_10       1   450 DAM       NaN     "Group: …          254
    ## 2 BAr1A1B1C_vRhyme_bin_10       1   450 NAT       NaN     "Group: …          254
    ## 3 BAr1A1B1C_vRhyme_bin_10       1   450 REST      NaN     "Group: …          254
    ## 4 BAr1A1B1C_vRhyme_bin_13       1   517 DAM        -0.333 "Group: …          254
    ## 5 BAr1A1B1C_vRhyme_bin_13       1   517 NAT         0.667 "Group: …          254
    ## 6 BAr1A1B1C_vRhyme_bin_13       1   517 REST       -0.333 "Group: …          254

### Plot Z-Score clusters

``` r
plot.clusters.crocach <-
ggplot(data = virus.clusters.crocach, aes(x = factor(treatment, levels = c("NAT", "REST", "DAM")), y = MeanZS)) +
  geom_boxplot(aes(color = treatment), outlier.shape = NA) +
  scale_color_manual(
        values = c("NAT" = "#4DAF4A", "REST" = "#377EB8", "DAM" = "#E41A1C"),
        name = "Ecosystem\nhealth status",
...
virus.clusters.langwell <- virus.clusters.langwell %>%
  ...
  mutate(Cluster.size = n_distinct(Virus)) %>%
  mutate(Title = factor(treatment, levels = c("NAT", "REST", "DAM")))
virus.clusters.langwell$Title <- paste("Group: ", virus.clusters.langwell$Cluster, "\n", virus.clusters.langwell$Cluster.size, " viruses", sep = "")
...
ggplot(data = virus.clusters.langwell, aes(x = factor(treatment, levels = c("NAT", "REST", "DAM")), y = MeanZS)) +
  geom_boxplot(aes(color = treatment), outlier.shape = NA) +
  scale_color_manual(
        values = c("NAT" = "#4DAF4A", "REST" = "#377EB8", "DAM" = "#E41A1C"),
        name = "Ecosystem\nhealth status",
...
virus.clusters.migneint <- virus.clusters.migneint %>%
  ...
  mutate(Cluster.size = n_distinct(Virus)) %>%
  mutate(Title = factor(treatment, levels = c("NAT", "REST", "DAM")))
virus.clusters.migneint$Title <- paste("Group: ", virus.clusters.migneint$Cluster, "\n", virus.clusters.migneint$Cluster.size, " viruses", sep = "")
...
ggplot(data = virus.clusters.migneint, aes(x = factor(treatment, levels = c("NAT", "REST", "DAM")), y = MeanZS)) +
  geom_boxplot(aes(color = treatment), outlier.shape = NA) +
  scale_color_manual(
        values = c("NAT" = "#4DAF4A", "REST" = "#377EB8", "DAM" = "#E41A1C"),
        name = "Ecosystem\nhealth status",
...
virus.clusters.moor_house <- virus.clusters.moor_house %>%
  ...
  mutate(Cluster.size = n_distinct(Virus)) %>%
  mutate(Title = factor(treatment, levels = c("NAT", "REST", "DAM")))
virus.clusters.moor_house$Title <- paste("Group: ", virus.clusters.moor_house$Cluster, "\n", virus.clusters.moor_house$Cluster.size, " viruses", sep = "")
...
ggplot(data = virus.clusters.moor_house, aes(x = factor(treatment, levels = c("NAT", "REST", "DAM")), y = MeanZS)) +
  geom_boxplot(aes(color = treatment), outlier.shape = NA) +
  scale_color_manual(
        values = c("NAT" = "#4DAF4A", "REST" = "#377EB8", "DAM" = "#E41A1C"),
        name = "Ecosystem\nhealth status",
...
virus.clusters.stean <- virus.clusters.stean %>%
  ...
  mutate(Cluster.size = n_distinct(Virus)) %>%
  mutate(Title = factor(treatment, levels = c("NAT", "DAM")))
virus.clusters.stean$Title <- paste("Group: ", virus.clusters.stean$Cluster, "\n", virus.clusters.stean$Cluster.size, " viruses", sep = "")
...
```
    ...
    ##   Virus                   Cluster order treatment  MeanZS Title     Cluster.size
    ##   <chr>                     <int> <int> <fct>       <dbl> <chr>            <int>
    ## 1 BAr1A1B1C_vRhyme_bin_16       1   261 DAM        -0.408 "Group: …          252
    ## 2 BAr1A1B1C_vRhyme_bin_16       1   261 REST        0.408 "Group: …          252
    ## 3 BAr1A1B1C_vRhyme_bin_21       1   312 DAM        -0.596 "Group: …          252
    ## 4 BAr1A1B1C_vRhyme_bin_21       1   312 REST        0.596 "Group: …          252
    ## 5 BAr1A1B1C_vRhyme_bin_27       1   221 DAM       NaN     "Group: …          252
    ## 6 BAr1A1B1C_vRhyme_bin_27       1   221 REST      NaN     "Group: …          252

### Plot Z-Score clusters

``` r
plot.clusters.stean <-
ggplot(data = virus.clusters.stean, aes(x = factor(treatment, levels = c("NAT", "REST", "DAM")), y = MeanZS)) +
  geom_boxplot(aes(color = treatment), outlier.shape = NA) +
  scale_color_manual(
        values = c("NAT" = "#4DAF4A", "REST" = "#377EB8", "DAM" = "#E41A1C"),
        name = "Ecosystem\nhealth status",
  ...
  xlab("Ecosystem health status") +
  ylab("Abundance\n(mean Z-Score)") +
  scale_x_discrete(labels = c("Restored", "Damaged")) +
  facet_wrap(~Title, ncol = 3, labeller = labeller(label_column = as.character)) +
  theme_linedraw() +
  theme(legend.position = "none", panel.grid.major.x = element_blank(), axis.text.x = element_text(angle = 45, hjust=1)) +
  ylim(-1,1) +
...
ggsave(plot = plot.clusters.combined,
       filename = "../Plots/DESeq/ExtendedDataFig3.png",
       device = "png",
       dpi = 600,
       width = 8,
       height = 10,
       ...
       bg = "white")
ggsave(plot = plot.clusters.combined,
       filename = "../Plots/DESeq/ExtendedDataFig3.svg",
       device = "svg",
       dpi = 600,
       width = 8,
       height = 10,
